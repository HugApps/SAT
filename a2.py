...
# assignment here 48
num_hours_i_spent_on_this_assignment = 48
#####################################################
#####################################################
...
# Assignments are pretty challenging, more explanation needed in lecture to explain the psuedo code for algorithms, for tihs case the sub routines are vague and confusing,
#####################################################
#####################################################
...
def dpll (instance,assignment):
    ...
    if has_empty_clause(instance) == True:
        ...
        if unit_clauses.__len__() >= 1:
         return dpll(simpliyfy(copy.deepcopy(instance),int(unit_clauses[0]),assignment),assignment)

        nextSymbol = instance.symbols.pop(0)
        assignment[nextSymbol] = 1

        positiveResult  = dpll(simpliyfy(copy.deepcopy(instance),nextSymbol,assignment) ,assignment)
        ...
        if positiveResult == True:
          ...
          assignment[nextSymbol] = -1
          negativeResult = dpll(simpliyfy(copy.deepcopy(instance),nextSymbol,assignment),assignment)
          print(negativeResult)
          return positiveResult or negativeResult
